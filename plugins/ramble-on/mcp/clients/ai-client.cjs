/**
 * mcp/clients/ai-client.cjs
 *
 * Unified AI client that runs in the privileged main process / MCP sidecar.
 * Adapts Gemini, OpenAI, and Anthropic behind a single async surface for
 * text generation, audio transcription, and Gemini video generation. The
 * renderer never touches raw API keys — it calls these capabilities via
 * MCP tools, which run here.
 */
...
/**
 * Transcribes audio. Anthropic does not offer transcription, so requests
 * with `provider: 'anthropic'` fall back to OpenAI then Gemini based on
...
/**
 * Generates a Veo video for the given prompt, polls until completion, and
 * downloads the result. The Gemini API key never crosses the IPC or MCP
 * boundary in a URL — the download happens here on the sidecar.
 *
